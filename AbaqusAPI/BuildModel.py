...
f = shelve.open('init_plain.dat')

# 保存的文件名，提交的计算作业名
mdbSaveName = f['mdbSaveName']
odbSaveName = f['odbSaveName']
savePath = f['abaqusFileSavePath']
os.chdir(savePath)
# 起吊点高度
time = f["time"]
...
myMaterial.Elastic(table=((25E+09, 0.28), ))
myMaterial.Density(table=((1800.0, ), ))
myModel.PipeProfile(name='P1', r=0.032/2, t=0.003)
...
set_loadMiddle = myAssembly.Set(
    name='Set-loadMiddle',
    vertices=Instance_B.vertices.getByBoundingBox(xMin=17.9,xMax=18.1,yMin=-0.1,yMax=0.1,zMin=-0.1,zMax=0.1)
    )
...
submitJob = mdb.Job(atTime=None,
        name=jobName,
        contactPrint=OFF,
        description='Generate by Python. Iteration time: %d'%time,
        echoPrint=OFF,
